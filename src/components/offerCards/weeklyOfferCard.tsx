...
export default function WeeklyOfferCard({ offer, className }: WeeklyOfferCardProps) {
    ...
    return (
        <div className="theme-pink">
            <div className={cn(
                "w-full min-h-[34rem] h-[34rem] sm:h-[33rem] md:h-[38.75rem] relative overflow-hidden border border-pink rounded-[2.5rem] bg-card-background mx-auto",
                className
            )}>
                {/* Discount Banner */}
                ...
                {/* Main Image */}
                <div className="relative h-[12.5rem] w-full bg-card-background sm:h-[15rem] border-b border-pink">
                    <img
                        src={image}
                        alt={title}
                    ...
                    <div className="min-w-[14.75rem] text-center absolute bottom-0 left-1/2 z-40 -translate-x-1/2 transform">
                        <div className="max-w-[14.5rem] min-w-[9rem] rounded-t-3xl text-sm shadow-lg px-[2rem] py-[0.34375rem] font-semibold lg:text-base border border-b-0 bg-card-background text-smoky-white border-pink w-full truncate">
                            {badge}
                        </div>
                    </div>
                </div>
                    ...
                    <div className="absolute bottom-[1.5rem] left-[1.5rem] right-[1.5rem] flex flex-col gap-2">
                        ...
                        <div className="grid grid-cols-7 gap-2 text-xs sm:text-sm lg:text-base font-semibold">
                            {allDays.map((day) => (
                                <div
                                    key={day}
                                    className={`flex items-center justify-center min-h-[1.4375rem] min-w-[2.018rem] py-1 rounded-sm ${availableDays.includes(day) ? 'bg-pink text-dark' : 'bg-card-background border border-pink text-smoky-white'
                                        }`}
                                >
                                    <p>{day}</p>
                                </div>
