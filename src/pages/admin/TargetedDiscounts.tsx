...
import { Tag, ArrowLeft, Plus, Trash2, Save, CheckCircle, XCircle } from "lucide-react";
...
export default function TargetedDiscountsPage() {
  ...
  const [discounts, setDiscounts] = useState<TargetedDiscount[]>([
    {
      id: "1",
      name: t("adminTargetedDiscounts.defaults.student"),
      createdAt: "2024-12-15",
      status: "active",
    },
    {
      id: "2",
      name: t("adminTargetedDiscounts.defaults.senior"),
      createdAt: "2024-12-10",
      status: "active",
    },
    {
      id: "3",
      name: t("adminTargetedDiscounts.defaults.firstTime"),
  ...
  const getStatusLabel = (status: DiscountStatus) =>
    t(`adminTargetedDiscounts.status.${status}`);

  const handleCreateDiscount = async () => {
    if (!discountName.trim()) {
      window.alert(t("adminTargetedDiscounts.notifications.nameRequired"));
      return;
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-white">
              {t("adminTargetedDiscounts.header.title")}
            </h1>
            <p className="text-gray-400 text-sm mt-1">
              {t("adminTargetedDiscounts.header.subtitle")}
            </p>
          </div>
        </div>
        ...
        >
          <Plus size={20} />
          {t("adminTargetedDiscounts.header.createButton")}
        </button>
      </div>
            ...
            {discounts.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-gray-400">
                  {t("adminTargetedDiscounts.empty.title")}
                </p>
                <p className="text-gray-500 text-sm mt-2">
                  {t("adminTargetedDiscounts.empty.description")}
                </p>
              </div>
            ) : (
                ...
                >
                  <div className="flex items-center gap-4 flex-1">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-gray-400 text-sm">
                        {t("adminTargetedDiscounts.list.createdOn", {
                          date: formatDate(discount.createdAt),
                        })}
                      </p>
                    </div>
                  ...
                  <div className="flex items-center gap-2 ml-4">
                    <button
                      ...
                      title={
                        discount.status === "active"
                          ? t("adminTargetedDiscounts.actions.deactivate")
                          : t("adminTargetedDiscounts.actions.activate")
                      }
                    >
                      {discount.status === "active" ? (
                        <XCircle size={18} />
                    ...
                    <button
                      ...
                      title={t("adminTargetedDiscounts.actions.delete")}
                    >
                      <Trash2 size={18} />
                    </button>
                  </div>
      ...
      <Modal
        ...
        title={t("adminTargetedDiscounts.modal.addTitle")}
      >
        <div className="space-y-4">
          <div>
            <label className="text-gray-400 text-sm mb-2 block">
              {t("adminTargetedDiscounts.form.nameLabel")} <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              value={discountName}
              ...
              placeholder={t("adminTargetedDiscounts.form.namePlaceholder") ?? ""}
              autoFocus
            />
            <p className="text-gray-500 text-xs mt-2">
              {t("adminTargetedDiscounts.form.nameHint")}
            </p>
          </div>
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-primary/30">
            ...
            >
              {t("adminTargetedDiscounts.modal.cancel")}
            </button>
            <button
            ...
            >
              {isSaving ? (
                <>
                  ...
                  {t("adminTargetedDiscounts.modal.creating")}
                </>
              ) : (
                <>
                  <Save size={18} />
                  {t("adminTargetedDiscounts.modal.createButton")}
                </>
              )}
            </button>
          </div>
      ...
      <Modal
        ...
        title={t("adminTargetedDiscounts.delete.title")}
      >
        <div className="space-y-4">
          <p className="text-gray-400">
            {t("adminTargetedDiscounts.delete.message")}
          </p>
          {showDeleteConfirm && (
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-primary/30">
            <button
              ...
              className="px-6 py-2 text-gray-400 hover:text-white hover:bg-primary/10 rounded-lg transition-all"
            >
              {t("adminTargetedDiscounts.delete.cancel")}
            </button>
            <button
            ...
            >
              {t("adminTargetedDiscounts.delete.confirmButton")}
            </button>
          </div>
        </div>
      </Modal>
